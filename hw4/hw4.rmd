...
First, let's zoom into two segments of the plot from 3a. Both plots show the overarching weekly trend, a relatively constant number of aircrafts over the weekdays with a substantial drop on Saturday and a partial recovery on Sunday. Both plots cover September and October, the top from 2000 (pre-9/11) and the bottom from 2007 (post-9/11). We can see that Tuesday in the 2007 plot feature less flights on average than the other weekdays; this is a difference in trend from the pre-9/11 plot.  
...
Below is a plot quantifying the average number of aircrafts per day of the week, showing the trend discussed above.  
...
all_days$date <- as.Date(paste(all_days$year, all_days$month, 
                          all_days$dayofmonth, sep = "-"), "%Y-%m-%d")

all_days %>%
  arrange(year, month, dayofmonth) %>% 
  mutate(day_of_week = wday(date)) %>%
  group_by(year, day_of_week) %>%
  summarize(sum_week = sum(n)) %>%
  collect()
```
