...
Let's see what seem like good predictors.

```{r, warning=FALSE, message=FALSE}
# Group by unique carrier:
model_data %>%
  group_by(uniquecarrier) %>%
  summarize(description = min(description), 
            mean_arrdelay = mean(arrdelay), 
            mean_depdelay = mean(depdelay), 
            mean_elaptime = mean(actualelapsedtime), 
            mean_dist = mean(distance)) %>%
  select(description, mean_arrdelay, 
         mean_depdelay, mean_elaptime, mean_dist) %>%
  arrange(desc(mean_arrdelay)) 
```
...
### Question: Were planes departing later on average after 9/11 as compared to before? Is there a difference between JFK (near the attack) and LAX (not near the attack)?
