...
Below is a plot quantifying the proportions of the number of total aircrafts per day of the week that enter and exit LAX grouped by year, showing the trend discussed above (excluding 1987, which does not have complete data for the year). Using proportions also allows us to see another subtle phenomenon - the lighter-colored (higher proportions) diagonal boxes moving up the plot from left to right signify the day of the week on which New Year's Day falls, which matches our plot from 3b, showing a spike in flights on average on January 1.  
